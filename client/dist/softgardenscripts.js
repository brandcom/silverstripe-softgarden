setTimeout(() => {
    ...
    const job_name_label = document.getElementById(
        "softgarden__job_name_label",
    );
    const job_info_label = document.getElementById(
        "softgarden__job_infos_label",
    );
    ...
    if (jobinfos) {
        ...
        if (softgarden_be_benefits) {
            ...
            try {
                ...
                const softgardenJobContent = document.querySelector(
                    ".softgarden-detailpage__posting-wrapper",
                ).innerHTML;
                //* Get the last ul Tag and push li tags in liElements
                const liElements = extractLiElements(softgardenJobContent);

                //* Benefits Limit
                const benefitTitle = document.getElementById(
                    "softgarden_benefit_title",
                );
                let benefitsLimit =
                    benefitTitle.getAttribute("data-MaxBenefits");
                if (benefitsLimit == "") {
                ...
                for (let i = 0; i < liElements.length; i++) {
                    ...
                    const benefitIndex = doesObjectExist(
                        benefit_list,
                        "benefit",
                        liElements[i],
                    );
                    if (benefitIndex !== -1) {
                        job_benefits.push({
                            benefit: `${liElements[i]}`,
                            benefit_icon_url: `${benefit_list[benefitIndex].benefit_icon_url}`,
                        });
                    } else {
                        try {
                            //! NOTE - It is important that there exists a benefit Data Object with the Benefit-Name "standard" and a default icon
                            const standardIndex = doesObjectExist(
                                benefit_list,
                                "benefit",
                                "standard",
                            );
                            job_benefits.push({
                                benefit: `${liElements[i]}`,
                                benefit_icon_url: `${benefit_list[standardIndex].benefit_icon_url}`,
                            });
                        } catch (error) {
                            console.log(error);
                        }
    ...
    function doesObjectExist(array, propertyName, value) {
        ...
        const targetValue = String(value).toUpperCase().trim();
        for (let i = 0; i < array.length; i++) {
            // Value in DataObject may contain multiple terms separated by semicolons
            const raw = String(array[i][propertyName] ?? "");
            ...
            if (candidates.includes(targetValue)) {
                return i; // Return the index if any term matches the current benefit
            }
        }
